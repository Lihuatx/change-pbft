...
# 初始化 NodeTable
node_table = {cluster: {f"{cluster}{i}": f"127.0.0.1:{base_port + i + (clusters.index(cluster) * nodes_per_cluster)}"
                        for i in range(nodes_per_cluster)}
              for cluster in clusters}

node_table_MP = {cluster: {f"{cluster}{i}": f"127.0.0.1:{base_port + i + (clusters.index(cluster) * nodes_per_cluster)}"
                           for i in range(nodes_per_cluster)}
                 for cluster in clusters}
